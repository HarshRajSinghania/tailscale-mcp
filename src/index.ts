...
if (subcommand === "deploy-acl" || subcommand === "validate-acl") {
  ...
  console.log(`Usage: tailscale-mcp [subcommand]

  tailscale-mcp deploy-acl <path-to-acl.json>
  tailscale-mcp validate-acl <path-to-acl.json>
  tailscale-mcp version

Running with no subcommand starts the MCP server on stdio.`);
  process.exit(0);
